"""
Created on Sunday October 12, 2025, 23:10:28
...
# Load the data 
    ...
    # Load the file using Pandas       
        # Resolve provided path: self.path may be a directory or a base path
        p = Path(self.path)
        if p.is_dir():
        ...
        # detect label column if present, else assume last column
        label_col = None
...
# Classification using the Gold Statndard after creating it from the raw text    
   ...
   # Get the data
        data,labels=self.get_data('winequality_white.csv')
        data=np.asarray(data)
...
# Experiments using training data only during training phase (dividing it into training and validation set)
        ...
        for train_index, test_index in skf.split(data,labels):
            ...
            if self.no_of_selected_features is not None:
                steps.append(('feature_selection', SelectKBest(chi2, k=self.no_of_selected_features)))
    ...
    # Evaluation
        ...
        if confidence_score>0.85:
            ...
            data,tst_cat=self.get_data('winequality_white.csv')
            tst_data=np.asarray(data)
            if trained_model is None:
                raise RuntimeError('No trained model available for making predictions')
            predicted = trained_model.predict(tst_data)
